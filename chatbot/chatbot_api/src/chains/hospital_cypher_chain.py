...
graph = Neo4jGraph(
    ...
    username=os.getenv("NEO4J_USERNAM"),
...
def consulta_neo4j(query:str) -> str:
    res = graph.query(query.replace('\n', ' '))
    return res

cypher_llm=ChatOpenAI(model=HOSPITAL_CYPHER_MODEL, temperature=0)
qa_llm=ChatOpenAI(model=HOSPITAL_QA_MODEL, temperature=0)

hospital_cypher_query= (
    {
...
runnable = RunnableParallel(
    context=lambda x: consulta_neo4j(hospital_cypher_query.invoke(x["question"])),
