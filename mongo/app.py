...
def auth_token(token: str= Depends(oauth2_scheme)):
    ...
    #Lógica de checagem de acesso
    if token == 'abcdefgh123':
        return 'adm_teste'
...
def auth_user(username, password):
    #chama api de plataformas
    if (username=='adm') & (password=='12345678'):
        return 'abcdefgh123'
    return None
...
async def login_for_access_token(
    ...
    #chama api de plataformas para obter token de acordo com usuário
    token = auth_user(form_data.username, form_data.password)
    if token:
...
async def create_student(student: StudentModel = Body(...), user: str | HTTPException= Depends(auth_token)):
    """
